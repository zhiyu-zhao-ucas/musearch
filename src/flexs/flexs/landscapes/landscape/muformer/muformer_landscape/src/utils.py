...
class EarlyStopping(object):
    def __init__(self, 
            ...
            least_loss=None, delta=1e-9, higher_better=True):
    ...
    def not_improved(self, val_score, val_loss):
        if np.isnan(val_score):
            return True
        if self.higher_better:
            # return (val_score < self.best_score + self.delta)
            return (val_score < self.best_score - self.delta) or ((val_score > self.best_score - self.delta) and (val_loss > self.least_loss - self.delta))
        else:
            # return val_score > self.best_score - self.delta
            return (val_score > self.best_score + self.delta) or ((val_score < self.best_score + self.delta) and (val_loss > self.least_loss - self.delta))
    
    def update(self, val_score, val_loss):
        # val_score = abs(val_score)
        if self.best_score is None:
            self.best_score = val_score
            self.least_loss = val_loss
            is_best = True
        elif self.not_improved(val_score,val_loss):
