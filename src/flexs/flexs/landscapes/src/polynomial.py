...
class PolynomialLRDecay(_LRScheduler):
    ...
    def __init__(self, optimizer, total_num_update, end_learning_rate=0.0, power=1.0, warmup_updates=0):
        if total_num_update <= 1.:
            raise ValueError('total_num_update should be greater than 1.')
    ...
    def get_lr(self):
        if self.last_step > self.total_num_update:
            ...
            return [(base_lr - self.end_learning_rate) * 
                    ((1 - (self.last_step-self.warmup_updates) / (self.total_num_update-self.warmup_updates)) ** (self.power)) + 
...
if __name__ == '__main__':
    ...
    scheduler = PolynomialLRDecay(optim, total_num_update=1000, end_learning_rate=1e-8, power=1.0,warmup_updates=100)
    a = []
